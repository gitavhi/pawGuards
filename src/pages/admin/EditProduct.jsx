...
export default function EditProduct() {
  ...
  useEffect(() => {
    if (product) {
      setForm({ name: product.name, category_id: product.category_id, description: product.description, price: product.price, stock: product.stock });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    try {
      db.updateProduct(id, { name: form.name, category_id: Number(form.category_id), description: form.description, price: Number(form.price), stock: Number(form.stock), image });
    ...
    <div className="admin-layout">
      ...
      <div className="admin-content">
        ...
        <form onSubmit={handleSubmit} className="admin-form">
          ...
          <div className="form-group"><label>Category</label><select name="category_id" className="form-control" value={form.category_id} onChange={handleChange} required>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
